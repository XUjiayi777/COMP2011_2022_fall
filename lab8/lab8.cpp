...
void warehousing(Fruit* fruit, Fruit* all_fruits){
    ...
    // after leaving the loop, temp points to the last fruit in the linked list
...
void stock_checking(const Fruit* all_fruits){
    for (const Fruit* i=all_fruits;i!=nullptr;i=i->next){
        cout<<i->name<<": "<<i->total_count<<endl;
    }
}
...
void stock_updating(Fruit* all_fruits, int fruit_id_to_sell, int number_to_sell){
    for (Fruit* i=all_fruits;i!=nullptr;i=i->next){
        if (i->id==fruit_id_to_sell){
            i->sold_count=number_to_sell;
            i->total_count-=number_to_sell;
        }
    }
...
float selling_amount_calculating(const Fruit* all_fruits){
    int money=0;
    for (const Fruit* i=all_fruits;i!=nullptr;i=i->next){
        money+=i->sold_count*i->unit_price;
    }
    return money;
